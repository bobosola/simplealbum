...
- **Bounded memory via the worker count.** Each generation job holds one frame in RAM — measured at ~100 MB for a 24 MP JPEG while it is decoded (the 69 MB RGB buffer plus the decoder's own planes) and ~55 MB at 12 MP — so total memory use is bounded by `[worker] threads`, never by the number of queued files or in-flight uploads. Measured peak RSS: 206 MB at 2 workers and 785 MB at 8 workers (24 MP photos), 453 MB at 8 workers (12 MP); 14 MB for a backlog of 8,000 small images; idle 12 MB watching 8,000 files. Note that the peak is retained rather than returned to the OS, so it is also the steady state. The job queue is bounded too: the initial scan and the watcher *park* when it is full rather than growing a list of tens of thousands of paths, so a large backlog adds latency (and briefly delays watcher events, which are re-derived by the next scan or event) but never memory. The pre-pass tasks are coroutines, not OS threads, and their number is capped independently of the queue depth. This count is the service's single memory lever: small servers pin a low value and keep a modest `MemoryMax`; larger machines leave it at `0` (auto).
